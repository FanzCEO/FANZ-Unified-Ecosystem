...
export function registerRoutes(app: Express) {
  ...
  app.get('/api/privacy/export', csrfProtection, isAuthenticated, async (req, res) => {
    try {
      ...
      res.setHeader('Content-Disposition', `attachment; filename=brofanz-data-export-${userId}.json`);
      res.json(exportData);
    } catch (error) {
  ...
  app.get('/api/compliance/custodian-notice', async (req, res) => {
    try {
      const custodianInfo = {
        ...
        notice: `Pursuant to 18 U.S.C. Section 2257, the following individual has been designated as the custodian of records for BroFanz platform:`,
        custodian: {
          name: 'BroFanz Legal Compliance Officer',
          company: 'BroFanz LLC',
          address: {
