...
Complete guide to deploying the BroFanz, GirlFanz, PupFanz, TransFanz, and TabooFanz platforms to Digital Ocean Kubernetes.
...
```bash
git clone https://github.com/FanzCEO/BroFanzV1.git
cd BroFanzV1
```
...
Go to: `https://github.com/FanzCEO/BroFanzV1/settings/secrets/actions`
...
For **each domain** (brofanz.com, girlfanz.com, etc.), add these DNS records:
...
# View logs
kubectl logs -f deployment/brofanz -n fanz-ecosystem
...
```bash
# Manual scaling
kubectl scale deployment brofanz --replicas=5 -n fanz-ecosystem
...
```bash
# Check certificate status
kubectl describe certificate brofanz-tls -n fanz-ecosystem

# Check cert-manager logs
kubectl logs -n cert-manager deploy/cert-manager

# Manually trigger certificate renewal
kubectl delete certificate brofanz-tls -n fanz-ecosystem
kubectl apply -f k8s/ingress/ingress.yaml
```
...
kubectl rollout history deployment/brofanz -n fanz-ecosystem

# Rollback to previous version
kubectl rollout undo deployment/brofanz -n fanz-ecosystem

# Rollback to specific revision
kubectl rollout undo deployment/brofanz --to-revision=2 -n fanz-ecosystem
```
...
curl https://brofanz.com/api/health
...
Expected response:
```json
{"status": "healthy", "platform": "BroFanz"}
```
...
- **GitHub Repository**: https://github.com/FanzCEO/BroFanzV1
