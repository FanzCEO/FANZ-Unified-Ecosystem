...
export class APIGatewayService extends EventEmitter {
  ...
  getHealthStatus(): { healthy: boolean; component: string; details?: any } {
    try {
      const metrics = this.getMetrics();
      const healthyRatio = metrics.services.total > 0 ? metrics.services.healthy / metrics.services.total : 1;
      return {
        healthy: healthyRatio >= 0.5,
