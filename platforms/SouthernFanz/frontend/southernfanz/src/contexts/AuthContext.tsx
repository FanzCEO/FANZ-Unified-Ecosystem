...
const mockApi = {
  login: async (email: string, password: string): Promise<{ user?: User; error?: string }> => {
    ...
    if (email === 'admin@southernfanz.com' && password === 'admin123') {
      return {
        user: {
          id: '1',
          username: 'admin',
    ...
    if (email === 'creator@southernfanz.com' && password === 'creator123') {
      return {
        user: {
          id: '2',
          username: 'alpha_muscle_god',
          email: 'creator@southernfanz.com',
    ...
    if (email === 'fan@southernfanz.com' && password === 'fan123') {
      return {
        user: {
          id: '3',
          username: 'underground_fan',
          email: 'fan@southernfanz.com',
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('southernfanz_user');
    if (savedUser) {
      try {
        setUser(JSON.parse(savedUser));
      } catch (error) {
        console.error('Failed to parse saved user:', error);
        localStorage.removeItem('southernfanz_user');
      }
    }
    setIsLoading(false);
  }, []);
  ...
  useEffect(() => {
    if (user) {
      localStorage.setItem('southernfanz_user', JSON.stringify(user));
    } else {
      localStorage.removeItem('southernfanz_user');
    }
  }, [user]);
