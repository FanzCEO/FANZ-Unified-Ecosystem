...
export class DatabaseStorage implements IStorage {
  ...
  async recordConsent(consent: {
    ...
    // Store consent in consent_records table using raw SQL for Supabase compatibility
    const { pool } = await import('./db');
    for (const [consentType, isGranted] of Object.entries(consent.consents)) {
      await pool.query(
        `INSERT INTO consent_records (session_id, user_id, consent_type, consent_version, is_granted, ip_address, user_agent, granted_at, created_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW())
         ON CONFLICT (session_id, consent_type) 
         DO UPDATE SET
           is_granted = EXCLUDED.is_granted,
           granted_at = CASE WHEN EXCLUDED.is_granted THEN EXCLUDED.granted_at ELSE consent_records.granted_at END,
           revoked_at = CASE WHEN NOT EXCLUDED.is_granted THEN NOW() ELSE consent_records.revoked_at END`,
        [
          consent.sessionId,
          consent.userId || null,
          consentType,
          ...
          isGranted ? consent.timestamp : null
        ]
      );
    }
  }
  ...
  async getConsent(sessionId: string): Promise<{
    ...
    // Look up consent from consent_records table using raw SQL for Supabase compatibility
    const { pool } = await import('./db');
    const result = await pool.query(
      'SELECT consent_type, is_granted FROM consent_records WHERE session_id = $1',
      [sessionId]
    );
    ...
    if (result.rows.length > 0) {
      ...
      for (const row of result.rows) {
        consents[row.consent_type] = row.is_granted;
      }
      return { consents };
    }
    return undefined;
