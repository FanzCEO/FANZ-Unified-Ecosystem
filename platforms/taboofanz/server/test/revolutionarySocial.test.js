...
async function testSocialMeetups() {
  ...
  try {
    ...
    const meetupData = {
      title: 'Exclusive Creator Meetup - TabooFanz Stars',
      description: 'Join top TabooFanz creators for an intimate networking session and Q&A.',
      type: 'CREATOR_MEETUP',
      format: 'HYBRID',
      capacity: 25,
      location: {
...
async function testVirtualEvents() {
  ...
  try {
    const eventData = {
      title: 'TabooFanz Pride Spectacular 2024',
      ...
      guestPerformers: [
        ...
        { name: 'Marcus Steel', type: 'HOST', bio: 'Popular TabooFanz creator and entertainer' }
      ],
      merchandise: [
...
async function testSocialGaming() {
  ...
  try {
    const gameData = {
      title: 'TabooFanz Trivia Championship',
