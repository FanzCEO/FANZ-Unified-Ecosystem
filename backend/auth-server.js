...
app.get('/api/status', optionalAuth, async (req, res) => {
  try {
    ...
    const tableStats = await dbConnection.query(`
      ...
      WHERE tablename IN ('vendors', 'vendor_access_tokens', 'admin_users')
      ORDER BY tablename
    `);
...
app.get('/api/vendors', async (req, res) => {
  try {
    ...
    const vendors = await dbConnection.query(`
      SELECT id, name, company, email, industry, description, website, 
...
app.get('/api/vendors/stats', async (req, res) => {
  try {
    ...
    const industryBreakdown = await dbConnection.query(`
