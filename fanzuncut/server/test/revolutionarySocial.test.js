...
async function testSocialMeetups() {
  ...
  try {
    ...
    const meetupData = {
      title: 'Exclusive Creator Meetup - FanzUncut Stars',
      description: 'Join top FanzUncut creators for an intimate networking session and Q&A.',
      type: 'CREATOR_MEETUP',
      format: 'HYBRID',
      capacity: 25,
      location: {
...
async function testVirtualEvents() {
  ...
  try {
    const eventData = {
      title: 'FanzUncut Pride Spectacular 2024',
      ...
      guestPerformers: [
        ...
        { name: 'Marcus Steel', type: 'HOST', bio: 'Popular FanzUncut creator and entertainer' }
      ],
      merchandise: [
...
async function testSocialGaming() {
  ...
  try {
    const gameData = {
      title: 'FanzUncut Trivia Championship',
