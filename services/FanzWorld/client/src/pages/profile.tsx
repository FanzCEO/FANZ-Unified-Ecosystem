...
export default function Profile() {
  ...
  const { data: allPosts = [] } = useQuery<PostWithAuthor[]>({
    queryKey: ["/api/posts"],
  });
  ...
  const getEmptyDescription = () => {
    switch (activeTab) {
      case 'posts':
        return "Start sharing your thoughts with the universe!";
      case 'replies':
        return "Join conversations by replying to posts!";
      case 'media':
        return "Share photos and videos with your posts!";
      case 'likes':
        return "Like posts to see them here!";
      default:
        return "Start engaging with the community!";
    }
  };
            ...
            <div className="relative">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-end space-y-4 sm:space-y-0 sm:space-x-4 -mt-16 sm:-mt-12">
                <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full border-4 border-cyber-blue bg-black overflow-hidden shadow-neon-cyan">
                  <img
                    src={user?.avatar || "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=128&h=128&fit=crop&crop=face"}
                ...
                <div className="flex-1">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4">
                    <div>
                      ...
                      <p className="text-gray-400">@{user?.username || "alex_chen"}</p>
                    </div>
                    <Button
                  ...
                  <p className="text-white mb-4 leading-relaxed">
                    {user?.bio || "Full-stack developer building the future"}
                  </p>
                  ...
                  <div className="flex flex-wrap items-center space-x-4 text-sm text-gray-400 mb-4">
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      <span className="text-cyber-blue">alexchen.dev</span>
                    </div>
                  </div>
                  
                  <div className="flex space-x-6 text-sm">
                    <div>
                      <span className="font-bold text-white">{user?.followingCount || 342}</span>
                      <span className="text-gray-400 ml-1">Following</span>
                    </div>
                    <div>
                      <span className="font-bold text-white">{user?.followersCount || 1250}</span>
