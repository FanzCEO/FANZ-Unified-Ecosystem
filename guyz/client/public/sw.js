// Guyz Progressive Web App Service Worker
// Advanced caching strategies, offline support, and background sync

const CACHE_NAME = 'Guyz-v1.0.0';
const OFFLINE_CACHE = 'Guyz-offline-v1.0.0';
const DYNAMIC_CACHE = 'Guyz-dynamic-v1.0.0';
const IMAGE_CACHE = 'Guyz-images-v1.0.0';
const API_CACHE = 'Guyz-api-v1.0.0';
...
self.addEventListener('install', (event) => {
  console.log('🚀 Guyz SW: Installing service worker');
    ...
    Promise.all([
      ...
      caches.open(OFFLINE_CACHE).then((cache) => {
        return cache.add('/offline.html').catch((error) => {
          console.warn('⚠️ Guyz SW: Failed to cache offline.html:', error);
      ...
      console.log('✅ Guyz SW: Installation complete');
      return self.skipWaiting();
    }).catch((error) => {
      console.error('❌ Guyz SW: Installation failed:', error);
...
async function cacheAssetsWithErrorHandling(cacheName, assets) {
  try {
    ...
    console.log('📦 Guyz SW: Caching app shell assets');
    ...
    const cachePromises = assets.map(async (asset) => {
      try {
        ...
        if (response.ok) {
          ...
          console.log('✅ Guyz SW: Cached:', asset);
        } else {
          console.warn(`⚠️ Guyz SW: Failed to fetch ${asset}: ${response.status}`);
        }
      } catch (error) {
        console.warn(`⚠️ Guyz SW: Error caching ${asset}:`, error.message);
        // Continue with other assets
      }
    });
    
    await Promise.allSettled(cachePromises);
    console.log('📦 Guyz SW: App shell caching completed');
  } catch (error) {
    console.error('❌ Guyz SW: Failed to open cache:', error);
  }
}
...
async function discoverAndCacheAssets() {
  if (isDevelopment) {
    console.log('🔧 Guyz SW: Development mode - skipping asset discovery');
    return;
  }
  ...
  try {
    ...
    if (indexResponse.ok) {
      ...
      console.log('🔍 Guyz SW: Discovered assets:', discoveredAssets);
      ...
      for (const asset of discoveredAssets) {
        try {
          ...
          if (response.ok) {
            await cache.put(asset, response);
            console.log('✅ Guyz SW: Cached discovered asset:', asset);
          }
        } catch (error) {
          console.warn(`⚠️ Guyz SW: Failed to cache ${asset}:`, error.message);
        }
      }
    }
  } catch (error) {
    console.warn('⚠️ Guyz SW: Asset discovery failed:', error.message);
  }
}

// Activate event - clean up old caches
self.addEventListener('activate', (event) => {
  console.log('🔄 Guyz SW: Activating service worker');
  ...
  event.waitUntil(
    Promise.all([
        ...
        return Promise.all(
          cacheNames.map((cacheName) => {
            if (cacheName !== CACHE_NAME && 
                ...
                cacheName !== API_CACHE) {
              console.log('🗑️ Guyz SW: Deleting old cache:', cacheName);
      ...
      console.log('✅ Guyz SW: Activation complete');
    })
  );
});
...
async function handleApiRequest(request) {
  ...
  try {
    ...
    console.log('🌐 Guyz SW: Network failed, trying cache for:', url.pathname);
...
async function handleImageRequest(request) {
  try {
    ...
    console.log('🖼️ Guyz SW: Image request failed:', request.url);
...
async function handleNavigationRequest(request) {
  try {
    ...
    console.log('🔍 Guyz SW: Navigation failed, checking cache');
...
self.addEventListener('sync', (event) => {
  console.log('🔄 Guyz SW: Background sync:', event.tag);
...
self.addEventListener('push', (event) => {
  console.log('📬 Guyz SW: Push notification received');
...
self.addEventListener('notificationclick', (event) => {
  console.log('📱 Guyz SW: Notification clicked:', event.action);
...
self.addEventListener('message', (event) => {
  console.log('💬 Guyz SW: Message received:', event.data);
...
async function queueOfflineAction(action) {
  try {
    ...
    console.log('📥 Guyz SW: Action queued for sync:', action.type);
  } catch (error) {
    console.error('❌ Guyz SW: Failed to queue action:', error);
  }
}
      ...
      if (!message.synced) {
        try {
          ...
          if (response.ok) {
            ...
            console.log('✅ Guyz SW: Message synced:', message.id);
          }
        } catch (error) {
          console.error('❌ Guyz SW: Failed to sync message:', error);
        }
      }
    }
  } catch (error) {
    console.error('❌ Guyz SW: Message sync failed:', error);
  }
}

async function syncQueuedLikes() {
  // Similar implementation for likes
  console.log('👍 Guyz SW: Syncing queued likes');
}

async function syncQueuedUploads() {
  // Similar implementation for content uploads
  console.log('📤 Guyz SW: Syncing queued uploads');
}

async function syncProfileUpdates() {
  // Similar implementation for profile updates
  console.log('👤 Guyz SW: Syncing profile updates');
}

async function clearSpecificCache(cacheName) {
  try {
    const deleted = await caches.delete(cacheName);
    console.log(`🗑️ Guyz SW: Cache ${cacheName} ${deleted ? 'deleted' : 'not found'}`);
  } catch (error) {
    console.error('❌ Guyz SW: Failed to clear cache:', error);
  }
}
...
async function updateAppBadge(count) {
  try {
    ...
    console.error('❌ Guyz SW: Failed to update badge:', error);
  }
}

async function openDB() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open('GuyzPWA', 1);
...
console.log('🚀 Guyz SW: Service Worker script loaded');