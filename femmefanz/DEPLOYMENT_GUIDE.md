...
Complete guide to deploying the FemmeFanz, GirlFanz, PupFanz, TransFanz, and TabooFanz platforms to Digital Ocean Kubernetes.
...
```bash
git clone https://github.com/FanzCEO/FemmeFanzV1.git
cd FemmeFanzV1
```
...
Go to: `https://github.com/FanzCEO/FemmeFanzV1/settings/secrets/actions`
...
For **each domain** (FemmeFanz.com, girlfanz.com, etc.), add these DNS records:
...
# View logs
kubectl logs -f deployment/FemmeFanz -n fanz-ecosystem
...
```bash
# Manual scaling
kubectl scale deployment FemmeFanz --replicas=5 -n fanz-ecosystem
...
```bash
# Check certificate status
kubectl describe certificate FemmeFanz-tls -n fanz-ecosystem

# Check cert-manager logs
kubectl logs -n cert-manager deploy/cert-manager

# Manually trigger certificate renewal
kubectl delete certificate FemmeFanz-tls -n fanz-ecosystem
kubectl apply -f k8s/ingress/ingress.yaml
```
...
kubectl rollout history deployment/FemmeFanz -n fanz-ecosystem

# Rollback to previous version
kubectl rollout undo deployment/FemmeFanz -n fanz-ecosystem

# Rollback to specific revision
kubectl rollout undo deployment/FemmeFanz --to-revision=2 -n fanz-ecosystem
```
...
curl https://FemmeFanz.com/api/health
...
Expected response:
```json
{"status": "healthy", "platform": "FemmeFanz"}
```
...
- **GitHub Repository**: https://github.com/FanzCEO/FemmeFanzV1
