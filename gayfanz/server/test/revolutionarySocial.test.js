...
async function testSocialMeetups() {
  ...
  try {
    ...
    const meetupData = {
      title: 'Exclusive Creator Meetup - GayFanz Stars',
      description: 'Join top GayFanz creators for an intimate networking session and Q&A.',
      type: 'CREATOR_MEETUP',
      format: 'HYBRID',
      capacity: 25,
      location: {
...
async function testVirtualEvents() {
  ...
  try {
    const eventData = {
      title: 'GayFanz Pride Spectacular 2024',
      ...
      guestPerformers: [
        ...
        { name: 'Marcus Steel', type: 'HOST', bio: 'Popular GayFanz creator and entertainer' }
      ],
      merchandise: [
...
async function testSocialGaming() {
  ...
  try {
    const gameData = {
      title: 'GayFanz Trivia Championship',
