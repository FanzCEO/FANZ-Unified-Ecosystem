...
export default function CardGlass({ title, description, children, className = '' }: CardGlassProps) {
  ...
  return (
    <div className={`relative rounded-sexy p-6 bg-ink-800/70 backdrop-blur-xs shadow-glass border border-white/10 ${className}`}>
      <div className="absolute inset-0 bg-grid opacity-[.05] pointer-events-none rounded-sexy" style={{ backgroundSize: '32px 32px' }}></div>
      <h3 className="font-display text-xl font-semibold tracking-flow text-white relative z-10">{title}</h3>
      <p className="text-sm text-slate-600 mt-2 relative z-10 leading-relaxed">{description}</p>
    <div
      ref={cardRef}
