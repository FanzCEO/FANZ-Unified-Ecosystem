...
export default function MobileUpload({ onPageChange }: MobileUploadProps) {
  const [uploadStep, setUploadStep] = useState<'capture' | 'metadata' | 'processing' | 'complete'>('capture');
  ...
  const handleCameraCapture = () => {
    // Simulate camera capture with a mock file
        ...
        {['📷', '📝', '⚙️', '✅'].map((icon, index) => (
          <div key={index} className="flex items-center">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${
              index <= ['capture', 'metadata', 'processing', 'complete'].indexOf(uploadStep) 
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium text-sm">Copyright Registration</p>
                    <p className="text-xs text-muted-foreground">Automated copyright filing</p>
                  </div>
                  <Switch 
                    checked={processingOptions.copyrightRegistration} 
                    onCheckedChange={(checked) => 
                      setProcessingOptions(prev => ({ ...prev, copyrightRegistration: checked }))
                    } 
                  />
                </div>
              ...
              <div className="space-y-2">
                <h4 className="text-sm font-medium">Output Formats</h4>
                <div className="grid grid-cols-2 gap-2">
                  {['MP4', 'WebM', 'MOV', 'JPEG', 'WebP', 'AVIF'].map((format) => (
                    <div key={format} className="flex items-center space-x-2">
                      <Checkbox 
                        id={format}
                        checked={processingOptions.outputFormats.includes(format)}
                        onCheckedChange={(checked) => {
                          if (checked) {
        ...
        <div className="space-y-6 text-center">
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Your content is protected and live on all platforms
              </p>
            </div>
          </div>
          ...
          <Card className="bg-green-50 border-green-200">
            <CardContent className="p-4">
              <div className="space-y-2 text-left">
                ...
                <div className="flex items-center space-x-2 text-sm">
                  <span>✅</span>
                  <span>Multi-format optimization complete</span>
                </div>
                <div className="flex items-center space-x-2 text-sm">
                  <span>✅</span>
                  <span>Content tracking enabled</span>
                </div>
              </div>
            </CardContent>
          </Card>
