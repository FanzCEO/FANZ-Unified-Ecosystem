...
Complete guide to deploying the BearFanz, GirlFanz, PupFanz, TransFanz, and TabooFanz platforms to Digital Ocean Kubernetes.
...
```bash
git clone https://github.com/FanzCEO/BearFanzV1.git
cd BearFanzV1
```
...
Go to: `https://github.com/FanzCEO/BearFanzV1/settings/secrets/actions`
...
For **each domain** (BearFanz.com, girlfanz.com, etc.), add these DNS records:
...
# View logs
kubectl logs -f deployment/BearFanz -n fanz-ecosystem
...
```bash
# Manual scaling
kubectl scale deployment BearFanz --replicas=5 -n fanz-ecosystem
...
```bash
# Check certificate status
kubectl describe certificate BearFanz-tls -n fanz-ecosystem

# Check cert-manager logs
kubectl logs -n cert-manager deploy/cert-manager

# Manually trigger certificate renewal
kubectl delete certificate BearFanz-tls -n fanz-ecosystem
kubectl apply -f k8s/ingress/ingress.yaml
```
...
kubectl rollout history deployment/BearFanz -n fanz-ecosystem

# Rollback to previous version
kubectl rollout undo deployment/BearFanz -n fanz-ecosystem

# Rollback to specific revision
kubectl rollout undo deployment/BearFanz --to-revision=2 -n fanz-ecosystem
```
...
curl https://BearFanz.com/api/health
...
Expected response:
```json
{"status": "healthy", "platform": "BearFanz"}
```
...
- **GitHub Repository**: https://github.com/FanzCEO/BearFanzV1
