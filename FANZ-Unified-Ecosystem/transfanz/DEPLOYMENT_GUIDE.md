...
Complete guide to deploying the TransFanz, GirlFanz, PupFanz, TransFanz, and TabooFanz platforms to Digital Ocean Kubernetes.
...
```bash
git clone https://github.com/FanzCEO/TransFanzV1.git
cd TransFanzV1
```
...
Go to: `https://github.com/FanzCEO/TransFanzV1/settings/secrets/actions`
...
For **each domain** (TransFanz.com, girlfanz.com, etc.), add these DNS records:
...
# View logs
kubectl logs -f deployment/TransFanz -n fanz-ecosystem
...
```bash
# Manual scaling
kubectl scale deployment TransFanz --replicas=5 -n fanz-ecosystem
...
```bash
# Check certificate status
kubectl describe certificate TransFanz-tls -n fanz-ecosystem

# Check cert-manager logs
kubectl logs -n cert-manager deploy/cert-manager

# Manually trigger certificate renewal
kubectl delete certificate TransFanz-tls -n fanz-ecosystem
kubectl apply -f k8s/ingress/ingress.yaml
```
...
kubectl rollout history deployment/TransFanz -n fanz-ecosystem

# Rollback to previous version
kubectl rollout undo deployment/TransFanz -n fanz-ecosystem

# Rollback to specific revision
kubectl rollout undo deployment/TransFanz --to-revision=2 -n fanz-ecosystem
```
...
Expected response:
```json
{"status": "healthy", "platform": "TransFanz"}
```
...
- **GitHub Repository**: https://github.com/FanzCEO/TransFanzV1
