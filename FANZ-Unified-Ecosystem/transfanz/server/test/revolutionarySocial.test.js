...
async function testSocialMeetups() {
  ...
  try {
    ...
    const meetupData = {
      title: 'Exclusive Creator Meetup - TransFanz Stars',
      description: 'Join top TransFanz creators for an intimate networking session and Q&A.',
      type: 'CREATOR_MEETUP',
      format: 'HYBRID',
      capacity: 25,
      location: {
...
async function testVirtualEvents() {
  ...
  try {
    const eventData = {
      title: 'TransFanz Pride Spectacular 2024',
      ...
      guestPerformers: [
        ...
        { name: 'Marcus Steel', type: 'HOST', bio: 'Popular TransFanz creator and entertainer' }
      ],
      merchandise: [
...
async function testSocialGaming() {
  ...
  try {
    const gameData = {
      title: 'TransFanz Trivia Championship',
