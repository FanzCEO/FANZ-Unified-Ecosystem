...
Complete guide to deploying the FanzUncut, GirlFanz, PupFanz, TransFanz, and TabooFanz platforms to Digital Ocean Kubernetes.
...
```bash
git clone https://github.com/FanzCEO/FanzUncutV1.git
cd FanzUncutV1
```
...
Go to: `https://github.com/FanzCEO/FanzUncutV1/settings/secrets/actions`
...
For **each domain** (FanzUncut.com, girlfanz.com, etc.), add these DNS records:
...
# View logs
kubectl logs -f deployment/FanzUncut -n fanz-ecosystem
...
```bash
# Manual scaling
kubectl scale deployment FanzUncut --replicas=5 -n fanz-ecosystem
...
```bash
# Check certificate status
kubectl describe certificate FanzUncut-tls -n fanz-ecosystem

# Check cert-manager logs
kubectl logs -n cert-manager deploy/cert-manager

# Manually trigger certificate renewal
kubectl delete certificate FanzUncut-tls -n fanz-ecosystem
kubectl apply -f k8s/ingress/ingress.yaml
```
...
kubectl rollout history deployment/FanzUncut -n fanz-ecosystem

# Rollback to previous version
kubectl rollout undo deployment/FanzUncut -n fanz-ecosystem

# Rollback to specific revision
kubectl rollout undo deployment/FanzUncut --to-revision=2 -n fanz-ecosystem
```
...
curl https://FanzUncut.com/api/health
...
Expected response:
```json
{"status": "healthy", "platform": "FanzUncut"}
```
...
- **GitHub Repository**: https://github.com/FanzCEO/FanzUncutV1
