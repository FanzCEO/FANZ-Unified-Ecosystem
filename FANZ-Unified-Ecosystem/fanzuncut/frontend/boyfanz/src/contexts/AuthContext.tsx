...
const mockApi = {
  login: async (email: string, password: string): Promise<{ user?: User; error?: string }> => {
    ...
    if (email === 'admin@FanzUncut.com' && password === 'admin123') {
      return {
        user: {
          id: '1',
          username: 'admin',
    ...
    if (email === 'creator@FanzUncut.com' && password === 'creator123') {
      return {
        user: {
          id: '2',
          username: 'alpha_muscle_god',
          email: 'creator@FanzUncut.com',
    ...
    if (email === 'fan@FanzUncut.com' && password === 'fan123') {
      return {
        user: {
          id: '3',
          username: 'underground_fan',
          email: 'fan@FanzUncut.com',
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('FanzUncut_user');
    if (savedUser) {
      try {
        setUser(JSON.parse(savedUser));
      } catch (error) {
        console.error('Failed to parse saved user:', error);
        localStorage.removeItem('FanzUncut_user');
      }
    }
    setIsLoading(false);
  }, []);
  ...
  useEffect(() => {
    if (user) {
      localStorage.setItem('FanzUncut_user', JSON.stringify(user));
    } else {
      localStorage.removeItem('FanzUncut_user');
    }
  }, [user]);
