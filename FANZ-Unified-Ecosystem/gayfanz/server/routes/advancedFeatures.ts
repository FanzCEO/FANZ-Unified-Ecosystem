  ...
  app.post('/api/nft/mint', isAuthenticated, csrfProtection, async (req, res) => {
    try {
      ...
      const nftAsset = await storage.createNftAsset({
        ...
        royaltyPercentage,
        status: 'minting',
        metadataUri: `https://GayFanz.app/api/nft/metadata/${mediaAssetId}`,
  ...
  app.get('/api/nft/metadata/:mediaAssetId', async (req, res) => {
    try {
      ...
      const metadata = {
        ...
        description: mediaAsset.description || `Exclusive content from GayFanz creator`,
        image: `https://GayFanz.app/api/media/${mediaAssetId}/preview`, // Blurred preview
        external_url: `https://GayFanz.app/media/${mediaAssetId}`,
        attributes: [
          {
