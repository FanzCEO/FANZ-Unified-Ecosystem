...
async function testSocialMeetups() {
  ...
  try {
    ...
    const meetupData = {
      title: 'Exclusive Creator Meetup - SouthernFanz Stars',
      description: 'Join top SouthernFanz creators for an intimate networking session and Q&A.',
      type: 'CREATOR_MEETUP',
      format: 'HYBRID',
      capacity: 25,
      location: {
...
async function testVirtualEvents() {
  ...
  try {
    const eventData = {
      title: 'SouthernFanz Pride Spectacular 2024',
      ...
      guestPerformers: [
        ...
        { name: 'Marcus Steel', type: 'HOST', bio: 'Popular SouthernFanz creator and entertainer' }
      ],
      merchandise: [
...
async function testSocialGaming() {
  ...
  try {
    const gameData = {
      title: 'SouthernFanz Trivia Championship',
