...
Create `src/services/discreteAPI.ts`:
...
export const discreteAPI = new DiscreteAPI();
```
...
const ccbillConfig = {
  ...
  formName: 'fanzdiscrete_load_card',
...
test('renders card with balance', () => {
  ...
  render(<DiscreteCard card={mockCard} />);
...
3. **Memoization**:
```typescript
const MemoizedCard = React.memo(DiscreteCard);
```
