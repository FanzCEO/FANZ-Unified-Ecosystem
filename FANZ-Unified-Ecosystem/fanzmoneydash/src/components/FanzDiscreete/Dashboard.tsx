...
const Dashboard: React.FC = () => {
  const [cards, setCards] = useState<DiscreteCardData[]>([]);
  const [selectedCard, setSelectedCard] = useState<DiscreteCardData | null>(null);
  ...
  const loadCards = async () => {
    try {
      setLoading(true);
      const response = await discreteAPI.getUserCards();
  ...
  const loadStats = async () => {
    try {
      const response = await discreteAPI.getSpendingSummary(30);
      setStats({
  ...
  const handleCreateCard = async () => {
    try {
      await discreteAPI.createCard('reloadable');
      await loadCards();
    } catch (error) {
            ...
            >
              {selectedCard ? (
                ...
                <div className="bg-white/5 backdrop-blur-lg rounded-3xl p-12 border border-white/10 text-center">
                  ...
                  <h3 className="text-2xl font-bold mb-2">No FanzDiscreete Card Yet</h3>
              ...
              <div className="space-y-3">
                <div className="flex items-start space-x-3">
                  ...
                  <p className="text-sm text-gray-300">
                    Load your FanzDiscreete card via CCBill
                  </p>
                </div>
                <div className="flex items-start space-x-3">
                  <div className="bg-purple-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold flex-shrink-0 mt-0.5">
