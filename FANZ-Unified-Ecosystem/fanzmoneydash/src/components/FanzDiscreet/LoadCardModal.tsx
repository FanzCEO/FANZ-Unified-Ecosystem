 ...
 * Multi-processor payment integration for loading funds onto FanzDiscreet cards
 */
...
import { PaymentProcessor } from '../../services/paymentProcessors';
...
type LoadStatus = 'selecting' | 'choosing_processor' | 'processing' | 'success' | 'error';
...
const LoadCardModal: React.FC<LoadCardModalProps> = ({ card, onClose, onComplete }) => {
  ...
  const loadCCBillConfig = async () => {
    try {
      const config = await discreetAPI.getCCBillConfig();
  ...
  const handleContinueToProcessorSelection = () => {
    ...
    setErrorMessage('');
    setLoadStatus('choosing_processor');
  };

  const handleProcessorSelect = (processor: PaymentProcessor) => {
    setSelectedProcessor(processor);
    setDescriptor(processor.discreet_billing.descriptor);
    setLoadStatus('processing');
    handleLoadCard(processor);
  };

  const handleLoadCard = async (processor: PaymentProcessor) => {
    setErrorMessage('');

    try {
      // Initialize payment session for selected processor
      const session = await discreetAPI.initializePaymentSession(
        card.card_id,
        selectedAmount,
        processor.id,
        'USD'
      ...
      if (processor.id === 'ccbill' && window.CCBillFlexForms && ccbillConfig) {
        ...
        // Redirect-based processors (Stripe, PayPal, etc.)
        window.location.href = session.redirect_url;
      } else if (session.payment_address) {
        // Cryptocurrency payment - show QR code / address
        handleCryptoPayment(session);
      } else {
        // Other processors - show payment form
        handleGenericPayment(session);
      }
    } catch (error: any) {
      setLoadStatus('error');
  ...
  const handleCryptoPayment = async (session: any) => {
    // For crypto, show the payment address and poll for confirmation
    setLoadStatus('processing');
    // In a real implementation, this would show a QR code and payment address
    // and poll the backend for payment confirmation

    // Simulate waiting for crypto confirmation
    const pollInterval = setInterval(async () => {
      try {
        const status = await discreetAPI.verifyPaymentSession(session.session_id);
        if (status.status === 'completed') {
          ...
          clearInterval(pollInterval);
          setLoadStatus('error');
          setErrorMessage(status.error || 'Crypto payment failed');
        }
      } catch (error) {
        clearInterval(pollInterval);
        setLoadStatus('error');
  ...
  const handleGenericPayment = async (session: any) => {
    // For wallet-based processors, complete the payment
    try {
      const result = await discreetAPI.loadCardMultiProcessor(
        card.card_id,
        selectedAmount,
        selectedProcessor!.id,
        { paymentToken: session.session_id }
      );
  ...
  const handlePaymentSuccess = async (response: any) => {
    try {
      ...
      if (selectedProcessor?.id === 'ccbill') {
        await discreetAPI.loadCard(
          card.card_id,
          selectedAmount,
          response.subscriptionId || response.transactionId
        );
      } else {
        await discreetAPI.loadCardMultiProcessor(
          card.card_id,
          selectedAmount,
          selectedProcessor!.id,
          { paymentToken: response.subscriptionId || response.transactionId }
        );
      }
  ...
  return (
    <AnimatePresence>
      {loadStatus === 'choosing_processor' ? (
        <ProcessorSelector
          amount_cents={selectedAmount}
          currency="USD"
          region="US"
          onSelect={handleProcessorSelect}
          onClose={() => setLoadStatus('selecting')}
        />
      ) : (
        <motion.div
          initial={{ opacity: 0 }}
            ...
            {loadStatus === 'selecting' && (
              <>
                ...
                <motion.button
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  onClick={handleContinueToProcessorSelection}
                ...
                <div className="mt-6 text-center">
                  <p className="text-xs text-gray-400">
                    20+ Payment Processors • 100% Discreet Billing
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    Choose from cards, crypto, wallets, and more
                  </p>
                </div>
              </>
            )}
            ...
            {loadStatus === 'success' && (
              ...
              >
                ...
                <p className="text-gray-400 mb-6">
                  ${(selectedAmount / 100).toFixed(2)} has been added to your FanzDiscreet card.
                </p>
