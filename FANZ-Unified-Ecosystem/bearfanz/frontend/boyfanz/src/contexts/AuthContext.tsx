...
const mockApi = {
  login: async (email: string, password: string): Promise<{ user?: User; error?: string }> => {
    ...
    if (email === 'admin@BearFanz.com' && password === 'admin123') {
      return {
        user: {
          id: '1',
          username: 'admin',
    ...
    if (email === 'creator@BearFanz.com' && password === 'creator123') {
      return {
        user: {
          id: '2',
          username: 'alpha_muscle_god',
          email: 'creator@BearFanz.com',
    ...
    if (email === 'fan@BearFanz.com' && password === 'fan123') {
      return {
        user: {
          id: '3',
          username: 'underground_fan',
          email: 'fan@BearFanz.com',
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('BearFanz_user');
    if (savedUser) {
      try {
        setUser(JSON.parse(savedUser));
      } catch (error) {
        console.error('Failed to parse saved user:', error);
        localStorage.removeItem('BearFanz_user');
      }
    }
    setIsLoading(false);
  }, []);
  ...
  useEffect(() => {
    if (user) {
      localStorage.setItem('BearFanz_user', JSON.stringify(user));
    } else {
      localStorage.removeItem('BearFanz_user');
    }
  }, [user]);
