  ...
  app.post('/api/nft/mint', isAuthenticated, csrfProtection, async (req, res) => {
    try {
      ...
      const nftAsset = await storage.createNftAsset({
        ...
        royaltyPercentage,
        status: 'minting',
        metadataUri: `https://BearFanz.app/api/nft/metadata/${mediaAssetId}`,
  ...
  app.get('/api/nft/metadata/:mediaAssetId', async (req, res) => {
    try {
      ...
      const metadata = {
        ...
        description: mediaAsset.description || `Exclusive content from BearFanz creator`,
        image: `https://BearFanz.app/api/media/${mediaAssetId}/preview`, // Blurred preview
        external_url: `https://BearFanz.app/media/${mediaAssetId}`,
        attributes: [
          {
