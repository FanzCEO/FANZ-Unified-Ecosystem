...
const mockApi = {
  login: async (email: string, password: string): Promise<{ user?: User; error?: string }> => {
    ...
    if (email === 'admin@Guyz.com' && password === 'admin123') {
      return {
        user: {
          id: '1',
          username: 'admin',
    ...
    if (email === 'creator@Guyz.com' && password === 'creator123') {
      return {
        user: {
          id: '2',
          username: 'alpha_muscle_god',
          email: 'creator@Guyz.com',
    ...
    if (email === 'fan@Guyz.com' && password === 'fan123') {
      return {
        user: {
          id: '3',
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('Guyz_user');
    if (savedUser) {
      try {
        setUser(JSON.parse(savedUser));
      } catch (error) {
        console.error('Failed to parse saved user:', error);
        localStorage.removeItem('Guyz_user');
      }
    }
    setIsLoading(false);
  }, []);
  ...
  useEffect(() => {
    if (user) {
      localStorage.setItem('Guyz_user', JSON.stringify(user));
    } else {
      localStorage.removeItem('Guyz_user');
    }
  }, [user]);
