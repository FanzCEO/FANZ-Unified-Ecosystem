...
Complete guide to deploying the Guyz, GirlFanz, PupFanz, TransFanz, and TabooFanz platforms to Digital Ocean Kubernetes.
...
```bash
git clone https://github.com/FanzCEO/GuyzV1.git
cd GuyzV1
```
...
For **each domain** (Guyz.com, girlfanz.com, etc.), add these DNS records:
...
```bash
# Check certificate status
kubectl describe certificate Guyz-tls -n fanz-ecosystem

# Check cert-manager logs
kubectl logs -n cert-manager deploy/cert-manager

# Manually trigger certificate renewal
kubectl delete certificate Guyz-tls -n fanz-ecosystem
kubectl apply -f k8s/ingress/ingress.yaml
```
...
kubectl rollout history deployment/Guyz -n fanz-ecosystem

# Rollback to previous version
kubectl rollout undo deployment/Guyz -n fanz-ecosystem

# Rollback to specific revision
kubectl rollout undo deployment/Guyz --to-revision=2 -n fanz-ecosystem
```
...
- **GitHub Repository**: https://github.com/FanzCEO/GuyzV1
