...
const mockApi = {
  login: async (email: string, password: string): Promise<{ user?: User; error?: string }> => {
    ...
    if (email === 'admin@TabooFanz.com' && password === 'admin123') {
      return {
        user: {
          id: '1',
          username: 'admin',
    ...
    if (email === 'creator@TabooFanz.com' && password === 'creator123') {
      return {
        user: {
          id: '2',
          username: 'alpha_muscle_god',
          email: 'creator@TabooFanz.com',
    ...
    if (email === 'fan@TabooFanz.com' && password === 'fan123') {
      return {
        user: {
          id: '3',
          username: 'underground_fan',
          email: 'fan@TabooFanz.com',
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('TabooFanz_user');
    if (savedUser) {
      try {
        setUser(JSON.parse(savedUser));
      } catch (error) {
        console.error('Failed to parse saved user:', error);
        localStorage.removeItem('TabooFanz_user');
      }
    }
    setIsLoading(false);
  }, []);
  ...
  useEffect(() => {
    if (user) {
      localStorage.setItem('TabooFanz_user', JSON.stringify(user));
    } else {
      localStorage.removeItem('TabooFanz_user');
    }
  }, [user]);
