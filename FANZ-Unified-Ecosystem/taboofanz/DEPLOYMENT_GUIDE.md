...
Complete guide to deploying the TabooFanz, GirlFanz, PupFanz, TransFanz, and TabooFanz platforms to Digital Ocean Kubernetes.
...
```bash
git clone https://github.com/FanzCEO/TabooFanzV1.git
cd TabooFanzV1
```
...
Go to: `https://github.com/FanzCEO/TabooFanzV1/settings/secrets/actions`
...
For **each domain** (TabooFanz.com, girlfanz.com, etc.), add these DNS records:
...
# View logs
kubectl logs -f deployment/TabooFanz -n fanz-ecosystem
...
```bash
# Manual scaling
kubectl scale deployment TabooFanz --replicas=5 -n fanz-ecosystem
...
```bash
# Check certificate status
kubectl describe certificate TabooFanz-tls -n fanz-ecosystem

# Check cert-manager logs
kubectl logs -n cert-manager deploy/cert-manager

# Manually trigger certificate renewal
kubectl delete certificate TabooFanz-tls -n fanz-ecosystem
kubectl apply -f k8s/ingress/ingress.yaml
```
...
kubectl rollout history deployment/TabooFanz -n fanz-ecosystem

# Rollback to previous version
kubectl rollout undo deployment/TabooFanz -n fanz-ecosystem

# Rollback to specific revision
kubectl rollout undo deployment/TabooFanz --to-revision=2 -n fanz-ecosystem
```
...
Expected response:
```json
{"status": "healthy", "platform": "TabooFanz"}
```
...
- **GitHub Repository**: https://github.com/FanzCEO/TabooFanzV1
